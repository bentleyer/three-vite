...
async function connect(odo, callback) {
    // Constable Odo takes many forms.
    /*
    ...
    */
    ...
    const requestDeviceRes = await window.navigator.hid.requestDevice({
        filters: [
            {
    ...
    device = requestDeviceRes[0];
    console.log('requestDeviceRes', requestDeviceRes);
    await device.open();
    ...
    const initDevice = async event => {
        try {
            ...
            if (data.length === 12) {
                ...
                await initWheel(device, options, callback);
            }
        } catch (err) {
            if (options.debug) {
                console.log('connect -> Error processing input report.', err);
            }
            callback(err);
        } finally {
            // 移除事件
            device.removeEventListener('inputreport', initDevice);
        }
    };
    device.addEventListener('inputreport', initDevice);
    ...
    //     }
    // };
    forceConstant(0.5); // move the wheel to generate a read event
} // connect

async function initWheel(device, options, callback) {
    try {
    // 发送初始化命令
        // await relayOS(device, [ 0xf8, 0x0a, 0x00, 0x00, 0x00, 0x00, 0x00 ]);
        // await relayOS(device, [ 0xf8, 0x09, 0x05, 0x01, 0x01, 0x00, 0x00 ]);
...
function relayOS(data) {
    ...
    */
    ...
    const reportId = 0x00;
    const sendData = new Uint8Array(data);
...
// Functions: Force
//------------------
function autoCenter(autocenter) {
    /*
    Set wheel autocentering based on existing options.
    */
    const option = autocenter || options.autocenter;
...
// Function: Listen
//------------------
async function listen(ready, callback) {
    /*
    ...
    */
    if (!ready) {
        device.close();
        const requestDeviceRes = await navigator.hid.requestDevice({ filters: [] }); // 根据需要设置过滤器
        device = requestDeviceRes[0];
    // device = new hid.HID(findWheel())
    }
    ...
    setRange();
    autoCenter();
    const inputreportFunc = event => {
    // const { device, reportId } = event;
        const data = new Uint8Array(event.data.buffer);
        // reset memory
        ...
        for (let i = 0; i < dataLength; i++) {
            // console.log('dataLength', data[i], dataPrev[i])
            if (data[i] !== dataPrev[i]) {
                dataDiffPositions.push(i);
            }
        }
        // if (dataDiffPositions.length > 0) {
        //     console.log('dataDiffPositions', data)
        // }

        // console.log('dataDiffPositions', dataDiffPositions, dataLength, data, dataPrev)
    ...
    device.addEventListener('inputreport', throttle(inputreportFunc, 50));
    // device.on("data", function(data) {

    // })
    device.addEventListener('error', function (err) {
        if (options.debug) {
...
export { connect, disconnect, eventEmitter, on, once, leds, forceConstant, forceFriction, forceOff, relayOS, autoCenter };
